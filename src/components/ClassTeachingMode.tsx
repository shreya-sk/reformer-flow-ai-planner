...
export const ClassTeachingMode = ({
  ...
  const getEnhancedCues = (exercise: Exercise): string[] => {
    ...
    const anatomicalCues = ["Engage deep abdominal muscles by drawing navel to spine", "Maintain neutral spine alignment with natural cervical curve", "Activate pelvic floor muscles throughout movement", "Keep shoulders away from ears, engaging latissimus dorsi", "Breathe laterally into ribcage on preparation, exhale on exertion"];
  ...
  return <div className="min-h-screen bg-gradient-to-br from-sage-50 via-white to-sage-100 relative overflow-hidden">
      ...
      <div className="absolute top-6 left-6 z-20">
        <Button onClick={onClose} variant="ghost" size="lg" className="bg-white/80 backdrop-blur-sm border border-sage-200 text-sage-700 hover:bg-white/90 hover:text-sage-800 rounded-2xl shadow-lg transition-all duration-300 transform hover:scale-105">
      ...
      <div className="absolute top-6 right-6 z-20">
        <div className="bg-white/80 backdrop-blur-sm border border-sage-200 rounded-2xl shadow-lg p-4">
          <div className="flex items-center gap-3">
            ...
            {isEditingGlobalTime ? <div className="flex items-center gap-2">
                <Input type="number" value={tempGlobalTime} onChange={e => setTempGlobalTime(parseInt(e.target.value) || 45)} className="w-20 h-8 text-sm border-sage-300 bg-white" min="1" max="120" />
                <Button onClick={handleGlobalTimeEdit} size="sm" variant="ghost" className="h-8 px-2 text-sm">
                  ✓
                </Button>
              </div> : <div className="flex items-center gap-2">
                <span className={`text-2xl font-bold ${globalTimeLeft < 300 ? 'text-red-500' : 'text-sage-800'}`}>
                  {formatTime(globalTimeLeft)}
                </span>
                <Button onClick={() => setIsEditingGlobalTime(true)} variant="ghost" size="sm" className="h-8 w-8 p-0 text-sage-600 hover:bg-sage-100">
      ...
      <div className="pt-24 pb-8 px-6 space-y-6 relative z-10">
        ...
        <div className="text-center space-y-3">
          ...
          <h1 className="font-bold text-sage-800 leading-tight text-4xl">
        ...
        <div className="text-center space-y-6">
          <div className="bg-white/80 backdrop-blur-sm p-8 border border-sage-200 shadow-xl max-w-sm mx-auto rounded-sm">
            {currentExercise.duration && currentExercise.duration > 0 ? <div className={`text-6xl font-bold ${exerciseTimeLeft < 30 ? 'text-red-500' : 'text-sage-800'}`}>
                {formatTime(exerciseTimeLeft)}
              </div> : <div className="text-3xl font-bold text-sage-800">
          ...
          <div className="flex items-center justify-center gap-4">
            <Button onClick={previousExercise} disabled={currentExerciseIndex === 0} variant="outline" size="lg" className="border-sage-300 hover:bg-sage-100 rounded-2xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50">
              <SkipBack className="h-6 w-6" />
            </Button>
            
            <Button onClick={handlePlayPause} size="lg" className="bg-gradient-to-r from-sage-600 to-sage-700 hover:from-sage-700 hover:to-sage-800 text-white px-8 py-4 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105">
              {isPlaying ? <Pause className="h-8 w-8" /> : <Play className="h-8 w-8" />}
            </Button>
            
            <Button onClick={nextExercise} disabled={currentExerciseIndex === exercises.length - 1} variant="outline" size="lg" className="border-sage-300 hover:bg-sage-100 rounded-2xl transition-all duration-300 transform hover:scale-105 disabled:opacity-50">
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-sage-200 shadow-lg rounded-2xl">
              ...
              <CardContent className="pt-0 space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {currentExercise.equipment.map((item, index) => <Badge key={index} variant="secondary" className="text-sm bg-sage-100 text-sage-700 px-3 py-1 rounded-full">
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-sage-200 shadow-lg rounded-2xl">
              ...
              <CardContent className="pt-0">
                <ul className="space-y-2">
                  {getEnhancedCues(currentExercise).map((cue, index) => <li key={index} className="text-sm leading-relaxed text-sage-700 flex items-start gap-3">
          ...
          <div className="space-y-4">
            ...
            <Card className="bg-white/80 backdrop-blur-sm border-sage-200 shadow-lg rounded-2xl">
              ...
              <CardContent className="pt-0 space-y-4">
                {/* Progressions */}
                {currentExercise.progressions && currentExercise.progressions.length > 0 && <div>
                    <h4 className="text-sm font-semibold text-green-600 mb-2">Make it Harder:</h4>
                    <ul className="space-y-1">
                      {currentExercise.progressions.slice(0, 3).map((progression, index) => <li key={index} className="text-sm text-sage-700 flex items-start gap-2">
                ...
                {/* Regressions */}
                {currentExercise.regressions && currentExercise.regressions.length > 0 && <div>
                    <h4 className="text-sm font-semibold text-blue-600 mb-2">Make it Easier:</h4>
                    <ul className="space-y-1">
                      {currentExercise.regressions.slice(0, 3).map((regression, index) => <li key={index} className="text-sm text-sage-700 flex items-start gap-2">
            ...
            {/* Safety Notes */}
            {currentExercise.contraindications && currentExercise.contraindications.length > 0 && <Card className="bg-white/80 backdrop-blur-sm border-amber-200 shadow-lg rounded-2xl">
                <CardHeader className="pb-3">
                ...
                <CardContent className="pt-0">
                  <ul className="space-y-1">
                    {currentExercise.contraindications.slice(0, 4).map((item, index) => <li key={index} className="text-sm text-sage-700 flex items-start gap-2">
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-sage-200 shadow-lg rounded-2xl max-w-4xl mx-auto">
          <CardContent className="p-6">
            {currentExercise.image ? <div className="relative h-64 rounded-2xl overflow-hidden">
              ...
              </div> : <div className="h-64 rounded-2xl bg-sage-50 flex flex-col items-center justify-center border-2 border-dashed border-sage-300">

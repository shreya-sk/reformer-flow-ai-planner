...
const Index = () => {
  ...
  return <div className={`min-h-screen ${preferences.darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-sage-25 via-white to-sage-50'}`}>
      ...
      <header className="relative overflow-hidden h-32">
        ...
        {/* Multiple organic wave layers for depth and flow */}
        <div className="absolute inset-0">
          {/* Primary flowing wave */}
          <svg className="absolute bottom-0 w-full h-28" viewBox="0 0 1440 320" preserveAspectRatio="none">
        ...
        <div className="relative px-6 py-4 flex items-center h-full">
          ...
          <div className="flex items-center space-x-5 ml-8">
            ...
            <div className="flex flex-col">
              <h1 style={{
              fontFamily: 'cursive'
            }} className="tracking-wide italic font-light text-sage-50 text-4xl px-0 py-[3px]">Welcome back,</h1>
      ...
      <div className="relative -mt-16 flex justify-center pr-4 mb-2 z-10 bg-transparent">
        <div className="relative group">
          <Avatar className="h-32 w-32 cursor-pointer transition-all duration-500 hover:scale-110 hover:rotate-2 border-4 border-white shadow-2xl relative z-10 bg-white" onClick={() => navigate('/profile')}>
            <AvatarImage src={preferences.profileImage} alt="Profile" className="rounded-full" />
            <AvatarFallback className="font-bold bg-gradient-to-br from-sage-400 to-sage-600 rounded-full mx-0 text-center text-4xl text-sage-200">
              {getUserInitials()}
            </AvatarFallback>
          </Avatar>
      ...
      {/* Enhanced Bottom Navigation with Organic Curves */}
      <div className="fixed bottom-0 left-0 right-0 z-50">
        <div className="relative overflow-hidden">
          ...
          {/* Flowing top wave */}
          <svg className="absolute top-0 w-full h-6" viewBox="0 0 1440 60" preserveAspectRatio="none">
            <path d="M0,60 C360,20 600,50 840,30 C1080,10 1320,40 1440,25 L1440,0 L0,0 Z" fill="rgba(255,255,255,0.1)" />
          </svg>
          
          <div className="relative flex items-center justify-around px-6 py-4 max-w-lg mx-auto">
            {/* Home */}
            <Button onClick={() => navigate('/')} variant="ghost" className="flex flex-col items-center text-white hover:text-sage-200 hover:bg-white/20 rounded-2xl transition-all duration-300 transform hover:scale-110">
              <Home className="h-6 w-6 mb-1" />
              <span className="text-xs font-medium">Home</span>
            </Button>
            
            {/* Library */}
            <Button onClick={() => navigate('/library')} variant="ghost" className="flex flex-col items-center text-white hover:text-sage-200 hover:bg-white/20 rounded-2xl transition-all duration-300 transform hover:scale-110">
              <BookOpen className="h-6 w-6 mb-1" />
              <span className="text-xs font-medium">Library</span>
            </Button>
            
            {/* Plan - Central Organic Button */}
            <Button onClick={() => navigate('/plan')} className="w-16 h-16 rounded-full bg-white/20 backdrop-blur-sm border-2 border-white/30 text-white hover:bg-white/30 shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-110 hover:rotate-12">
              <Plus className="h-8 w-8" />
            </Button>
            
            {/* Timer */}
            <Button onClick={() => navigate('/timer')} variant="ghost" className="flex flex-col items-center text-white hover:text-sage-200 hover:bg-white/20 rounded-2xl transition-all duration-300 transform hover:scale-110">
              <Timer className="h-6 w-6 mb-1" />
              <span className="text-xs font-medium">Timer</span>
            </Button>
            
            {/* Profile */}
            <Button onClick={() => navigate('/profile')} variant="ghost" className="flex flex-col items-center text-white hover:text-sage-200 hover:bg-white/20 rounded-2xl transition-all duration-300 transform hover:scale-110">
              <div className="w-6 h-6 rounded-full bg-white/30 mb-1 flex items-center justify-center">
      ...
      <div className="p-4 space-y-3 pb-28">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between px-2">
            ...
            {savedClasses.length > 0 && <p className={`text-sm ${preferences.darkMode ? 'text-gray-400' : 'text-sage-600'} bg-sage-100 dark:bg-gray-700 px-3 py-1 rounded-full`}>
                {savedClasses.length} class{savedClasses.length === 1 ? '' : 'es'}
              </p>}
          </div>

          {savedClasses.length === 0 ? <div className={`text-center py-8 px-6 ${preferences.darkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-lg`}>
              ...
              <Button onClick={() => navigate('/plan')} className="bg-gradient-to-r from-sage-600 to-sage-700 hover:from-sage-700 hover:to-sage-800 text-white rounded-full px-6 py-2 transform hover:scale-105 transition-all duration-300 shadow-lg">
                <Plus className="h-4 w-4 mr-2" />
                Create Class
              </Button>
            </div> : <ClassPlanList classes={savedClasses} onEditClass={handleEditClass} onDeleteClass={handleDeleteClass} />}
        </div>
      </div>
    </div>;
};
